...
var (
	...
	ErrClosed = errors.New("netpool: client is closed")

	// ErrPoolTimeout timed out waiting to get a connection from the connection pool.
	ErrPoolTimeout = errors.New("netpool: connection pool timeout")
)
...
func (p *ConnPool) reaper(frequency time.Duration) {
	...
	for {
		...
		case <-ticker.C:
			...
			if err != nil {
				fmt.Println(context.Background(), "ReapStaleConns failed: %s", err)
				continue
			}
		case <-p.closedCh:
			return
